...
export default function BlogCategorySection() {
    ...
    useEffect( ()=>{
        ...
        (async ()=>{
            setLoading(true)
            try {
                const {data} = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/blogCategory/getAll`,{
  ...
  const handleDeleteAction= async(item,state)=>{
    try {
      ...
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/blogCategory/delete/${item._id}/${item.image?.public_id|| 0}`
      );
